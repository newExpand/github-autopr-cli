...
- GitHub PR 자동 리뷰 기능 추가 (실험적)
  - 새로운 `review-bot` 명령어로 GitHub Actions 통합
  - PR 생성 및 업데이트 시 자동 코드 리뷰 제공
  - 전체 PR에 대한 종합적인 코드 리뷰 요약 생성
  - 특정 코드 라인에 대한 인라인 코멘트 제공
  - 개선 권장사항 및 모범 사례 제안
  - 코드 품질, 버그, 보안 문제 등을 자동으로 감지
  - 다국어 지원으로 사용자 로케일에 맞는 코멘트 생성
  - 초기화 과정에서 GitHub Actions 워크플로우 자동 설정
  - PR 변경 내용 분석을 위한 AI 기능 최적화
  - 리뷰 결과의 정확한 diff 위치 매핑 알고리즘 구현
