...
THE MOST IMPORTANT REQUIREMENT: Every review MUST include a "Changed Files Summary" section with a proper Markdown table listing all modified files with their type of change, lines added/removed, and a brief summary.
...
Your review MUST include the following sections in this exact order:
...
2. **Changed Files Summary**: A Markdown table with the following columns:
   ...
   You MUST include this table in the following format - THIS TABLE IS MANDATORY:
   ...
   Include ALL changed files in this table, without exception. This section CANNOT be omitted.
...
CRITICAL: You MUST include the "Changed Files Summary" table with information about ALL changed files.
...
Return only a raw JSON array.`;
      ...
      for (const file of context.changedFiles) {
        if (!file.content) {
          log.debug(`파일 ${file.path}의 내용이 없습니다.`);
          continue;
        }

        if (!fileChanges[file.path] || fileChanges[file.path].length === 0) {
          log.debug(`파일 ${file.path}의 변경된 라인 정보를 찾을 수 없습니다.`);
          continue;
        }
            ...
            if (Array.isArray(comments)) {
              for (const comment of comments) {
                if (comment.lineNumber && comment.comment) {
                  ...
                  if (lineInfo && lineInfo.position > 0) {
                    lineComments.push({
                      path: file.path,
